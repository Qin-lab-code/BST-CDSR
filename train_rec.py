...
parser.add_argument('--semantic_dim', type=int, default=512,
                    help='Dimension of semantic embeddings after PCA.')
parser.add_argument('--top_k', type=int, default=5)

# model part
parser.add_argument('--model', type=str, default="BSTCDSR", help='model name')
parser.add_argument('--hidden_units', type=int, default=256, help='lantent dim.')
parser.add_argument('--num_blocks', type=int, default=1, help='lantent dim.')
...
parser.add_argument('--sem_loss_weight', type=float, default=1e-2)
parser.add_argument('--ode_loss_weight', type=float, default=1e-3)
early_stop = 10
...
train_batch = DataLoader(opt['data_dir'], opt['batch_size'], opt,
                         evaluation=-1)  # 134, 18, 256, 15 (batch_num, data_types, batch_size, seq_length)
...
for epoch in range(1, opt['num_epoch'] + 1):
    ...
    trainer.mi_loss = 0
    global_step, train_loss = trainer.train(global_step, train_batch)
    ...
    with torch.no_grad():
        ...
        test_X_MRR, test_X_NDCG_5, test_X_NDCG_10, test_X_HR_1, test_X_HR_5, test_X_HR_10 = cal_test_score(
            test_X_pred
        )
        test_Y_MRR, test_Y_NDCG_5, test_Y_NDCG_10, test_Y_HR_1, test_Y_HR_5, test_Y_HR_10 = cal_test_score(
            test_Y_pred
        )
        ...
        if test_X_MRR > best_x_1:
            best_x_1 = test_X_MRR
            improved_x = True
        if test_X_NDCG_5 > best_x_2:
            best_x_2 = test_X_NDCG_5
            improved_x = True
        if test_X_NDCG_10 > best_x_3:
            best_x_3 = test_X_NDCG_10
            improved_x = True
        if test_X_HR_1 > best_x_4:
            best_x_4 = test_X_HR_1
            improved_x = True
        if test_X_HR_5 > best_x_5:
            best_x_5 = test_X_HR_5
            improved_x = True
        if test_X_HR_10 > best_x_6:
            best_x_6 = test_X_HR_10
            improved_x = True

        if test_Y_MRR > best_x_7:
            best_x_7 = test_Y_MRR
            improved_x = True
        if test_Y_NDCG_5 > best_x_8:
            best_x_8 = test_Y_NDCG_5
            improved_x = True
        if test_Y_NDCG_10 > best_x_9:
            best_x_9 = test_Y_NDCG_10
            improved_x = True
        if test_Y_HR_1 > best_x_10:
            best_x_10 = test_Y_HR_1
            improved_x = True
        if test_Y_HR_5 > best_x_11:
            best_x_11 = test_Y_HR_5
            improved_x = True
        if test_Y_HR_10 > best_x_12:
            best_x_12 = test_Y_HR_10
            improved_x = True

        if improved_x:
            print("X best")
            print([best_x_1, best_x_2, best_x_3, best_x_4, best_x_5, best_x_6])

        improved_y = False

        if test_X_MRR > best_y_1:
            best_y_1 = test_X_MRR
            improved_y = True
        if test_X_NDCG_5 > best_y_2:
            best_y_2 = test_X_NDCG_5
            improved_y = True
        if test_X_NDCG_10 > best_y_3:
            best_y_3 = test_X_NDCG_10
            improved_y = True
        if test_X_HR_1 > best_y_4:
            best_y_4 = test_X_HR_1
            improved_y = True
        if test_X_HR_5 > best_y_5:
            best_y_5 = test_X_HR_5
            improved_y = True
        if test_X_HR_10 > best_y_6:
            best_y_6 = test_X_HR_10
            improved_y = True

        if test_Y_MRR > best_y_7:
            best_y_7 = test_Y_MRR
            improved_y = True
        if test_Y_NDCG_5 > best_y_8:
            best_y_8 = test_Y_NDCG_5
            improved_y = True
        if test_Y_NDCG_10 > best_y_9:
            best_y_9 = test_Y_NDCG_10
            improved_y = True
        if test_Y_HR_1 > best_y_10:
            best_y_10 = test_Y_HR_1
            improved_y = True
        if test_Y_HR_5 > best_y_11:
            best_y_11 = test_Y_HR_5
            improved_y = True
        if test_Y_HR_10 > best_y_12:
            best_y_12 = test_Y_HR_10
            improved_y = True

        if improved_y:
            print("Y best")
            print([best_y_7, best_y_8, best_y_9, best_y_10, best_y_11, best_y_12])

        if improved_x or improved_y:
            count = 0
        else:
            count += 1
            print("count:", count)
...
out_file = f"results{args.top_k}.csv"
with open(out_file, mode='w', newline='') as file:
    ...
    writer.writerow(
        [round(best_x_1 * 100, 2), round(best_x_2 * 100, 2), round(best_x_3 * 100, 2),
         round(best_x_4 * 100, 2), round(best_x_5 * 100, 2), round(best_x_6 * 100, 2),
         round(best_x_7 * 100, 2), round(best_x_8 * 100, 2), round(best_x_9 * 100, 2),
         round(best_x_10 * 100, 2), round(best_x_11 * 100, 2), round(best_x_12 * 100, 2)])

    writer.writerow(
        [round(best_y_1 * 100, 2), round(best_y_2 * 100, 2), round(best_y_3 * 100, 2),
         round(best_y_4 * 100, 2), round(best_y_5 * 100, 2), round(best_y_6 * 100, 2),
         round(best_y_7 * 100, 2), round(best_y_8 * 100, 2), round(best_y_9 * 100, 2),
         round(best_y_10 * 100, 2), round(best_y_11 * 100, 2), round(best_y_12 * 100, 2)])

print(f"Results have been saved to {out_file}.")
